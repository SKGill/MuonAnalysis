...
struct Loader {
  ...
  double weight;
  double charge[2], chi2dof[2], pixel_hits[2], strip_hits[2], pixel_layers[2], strip_layers[2], tracker_layers[2], muon_hits[2], muon_hits_DT[2], muon_hits_CSC[2], muon_hits_RPC[2];
  double pt[2], inv_pt[2], qpt[2], qinv_pt[2], theta[2], eta[2], phi[2], dxy[2], dz[2], dptOpt[2];
  ...
  Loader(const CosmicSplittingResolutionNtuple* nt,
	...
	jn = use_in_staglb;
      ...
      tracker_layers[jm] = nt->pixel_layers[in][jn] + nt->strip_layers[in][jn];
      muon_hits[jm]    = nt->muon_hits[in][jn];
//       muon_hits_DT[jm]    = nt->muon_hits_DT[in][jn];
//       muon_hits_CSC[jm]    = nt->muon_hits_CSC[in][jn];
//       muon_hits_RPC[jm]    = nt->muon_hits_RPC[in][jn];
	...
	qinv_pt[jm]      = charge[jm] * inv_pt[jm];
        eta[jm]          = -log(tan(nt->unprop_theta[in][jn]/2));
	...
	dz_error[jm]     = nt->unprop_error_dz[in][jn];
        dptOpt[jm]       = pt_error[jm] * inv_pt[jm];
      }
      else {
	charge[jm]       = q * nt->charge[in][jn];
        ...
        qpt[jm]          = charge[jm] * pt[jm];
        qinv_pt[jm]      = charge[jm] * inv_pt[jm];
        eta[jm]          = -log(tan(nt->theta[in][jn]/2));
        theta[jm]        = nt->theta[in][jn];
        phi[jm]          = nt->phi[in][jn];
        dxy[jm]          = nt->unprop_dxy[in][jn]; // not filled correctly for not-unprop
        dz[jm] 	         = nt->unprop_dz[in][jn];
      ...
      mc_eta      = -log(tan(nt->mc_theta[track]/2));
...
struct Bin {
      ...
      ref_ok(track == tk_tkonly)
  {
    ...
    h_tracker_layers = fac.Create("trackerlayers",   15,  0, 15);
    h_muon_hits    = fac.Create("muonhits",      60,  0, 60);
    h_muon_hits_DT = fac.Create("muonhits_DT",   60,  0, 60);
    h_muon_hits_CSC= fac.Create("muonhits_CSC",  60,  0, 60);
    h_muon_hits_RPC= fac.Create("muonhits_RPC",  60,  0, 60);
    ...
    h_eta          = fac.Create("eta",    200/nbins_scale,  -3.,  3.);
    ...
    h_dxy          = fac.Create("dxy",    200/nbins_scale, -50., 50.,      100/nbins_scale, -diff_scales[4], diff_scales[4], -res_scales[4], res_scales[4], -pull_scales[4], pull_scales[4]);
    h_dz           = fac.Create("dz",     200/nbins_scale, -80., 80.,      100/nbins_scale, -diff_scales[5], diff_scales[5], -res_scales[5], res_scales[5], -pull_scales[5], pull_scales[5]);
    h_dptOpt       = fac.Create("dptOpt", 1000, 0., 5.);
  ...
  ~Bin() {
    ...
    delete h_tracker_layers;
    delete h_muon_hits;
    delete h_muon_hits_DT;
    delete h_muon_hits_CSC;
    delete h_muon_hits_RPC;
    delete h_charge;
    delete h_pt;
    delete h_inv_pt;
    delete h_qpt;
...
//     if (nt->ref_pt <10.) return false;
  ...
  bool check_and_fill(const CosmicSplittingResolutionNtuple* nt, std::ostringstream* out=0) {
    ...
    for (int j = 0; j < 2; ++j) {
      ...
      h_tracker_layers ->Fill(loader.tracker_layers, 0, loader.weight);
      h_muon_hits      ->Fill(loader.muon_hits,    0, loader.weight);
      h_muon_hits_DT   ->Fill(loader.muon_hits_DT,  0, loader.weight);
      h_muon_hits_CSC  ->Fill(loader.muon_hits_CSC, 0, loader.weight);
      h_muon_hits_RPC  ->Fill(loader.muon_hits_RPC, 0, loader.weight);
      ...
      h_eta    ->Fill(loader.eta,     0,                   loader.weight, loader.mc_eta     );
      ...
      h_dptOpt ->Fill(loader.dptOpt,  0,                   loader.weight);
...
CosmicSplittingResolutionHistos::CosmicSplittingResolutionHistos(const edm::ParameterSet& cfg)
  : is_mc(cfg.getParameter<bool>("is_mc")),
    ...
    max_dxy(cfg.getParameter<double>("max_dxy")),
    max_dz(cfg.getParameter<double>("max_dz")),    
    ...
    copy_selected_events(cfg.getParameter<bool>("copy_selected_events"))
{
  ...
  errors->GetXaxis()->SetBinLabel(1 + error_dpt,                 "dpt/pt");
  errors->GetXaxis()->SetBinLabel(1 + error_pixels,              "pixels");
  errors->GetXaxis()->SetBinLabel(1 + error_strips,              "strips");
  errors->GetXaxis()->SetBinLabel(1 + error_dxy,                 "dxy");
  errors->GetXaxis()->SetBinLabel(1 + error_dz,                  "dz");
...
CosmicSplittingResolutionHistos::error_code CosmicSplittingResolutionHistos::cut() {
  ...
  if (min_muon_hits > 0)
    for (int j = 0; j < 2; ++j) {
      if (nt->muon_hits[tk_global][j] < min_muon_hits){ // || nt->muon_hits_RPC[tk_global][j] < 1) {
  ...
  if (min_pixel_layers > 0 || min_strip_layers > 0)
	...
	continue;
      for (int j = 0; j < 2; ++j) {
//         if (!(nt->phi[i][j] < (-3.14156/2. - 3.14156/4.) || nt->phi[i][j] > (-3.14156/2. + 3.14156/4.))) return error_propagation;
	...
	if (nt->strip_layers[i][j] < min_strip_layers) {
	  ...
	  return error_strips;
        ...
        if (fabs(nt->dz[i][j]) > max_dz) {
          out << "Event failed selection " << j << ": dz: " << nt->dz[i][j] << "/" << max_dz << "\n";
          return error_dz;
        }
        
        // High-pt-ID
//         if (nt->pixel_hits[i][j] < min_pixel_layers) {
//           out << "Event failed selection on track " << i << "," << j << ": pixel layers: " << nt->pixel_layers[i][j] << "/" << min_pixel_layers << "\n";
//           return error_pixels;
//         }
//         if (nt->pixel_layers[i][j] + nt->strip_layers[i][j] < min_strip_layers) {
//           out << "Event failed selection on track " << i << "," << j << ": strip layers: " << nt->strip_layers[i][j] << "/" << min_strip_layers << "\n";
//           return error_strips;
//         }
  ...
  for (int i = 0; i < n_tracks; i++){
    for (int j = 0; j < 2; j++){
      if (nt->error_pt[i][j] / nt->pt[i][j] >= 0.3){
        out << "Event failed selection on track " << i << "," << j << ": dpt / pt " << nt->error_pt[i][j] / nt->pt[i][j] << "/ 0.3" << "\n";
//         return error_dpt;
      }
    }
  }
  ...
  for (int i = 0; i < 5; ++i) {
    if (i == tk_stalone)
      continue; 
    for (int j = 0; j < 2; ++j) {
      //std::cout << i << " " << fabs(nt->unprop_dxy[i][j]) << " " << max_dxy << std::endl;
      if (fabs(nt->unprop_dxy[i][j]) > max_dxy) {
        
        out << "Event failed selection  " << j << ": dxy unprop global: " << nt->unprop_dxy[tk_global][j] << "\n";
        return error_dxy;
      }
    }
  }
